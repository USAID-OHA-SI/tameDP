...
Now that you have the package loaded, you can start rolling with getting out of the Data Pack in a tidy manner.

The first think you will want to do is get the full file path to your Data Pack. In your Windows Explorer, navigate to your Data Pack, and hold the SHIFT key while you right click. Doing this gives you an additional option to "Copy as path", which you want to click. We can store this in R as an object using the assignment operator `<-` and then use this later on, passing it in to different functions. Replace the file path below with the actual filepath to yoru Data Pack.

```{r filepath}
dp_filepath <- "C:/Users/spower/Downloads/DataPack_Jupiter_20500101.xlsx"
```

There are a few different forms of data we can get out of the Data Pack. The first is extracting all the targets from their relevant tabs. The data here will be at the PSNU level, broken out by the relevant disagregates (e.g. Age/Sex/HIVStatus). In the function below, we have used the filepath we stored above to pass that into the function.

```{r base function}
df_dp <- tame_dp(dp_filepath)
```

This function will take a few seconds to run and will store your data in an objected called `df_dp`. If you browse the data frame (df) using a function like `View(df_dp)` or `dplyr::glimpse(df_dp)`, you'll see the structure is very similar to a MER Structured Dataset (MSD). You have the following columns: `operatingunit, countryname, snu1, psnu, psnuuid, fiscal_year, indicator, standardizeddisaggregate, numeratordenom, ageasentered, sex, modality, statushiv, otherdisaggregate, targets`. The output is a bit more manageable to work with than using it in the Data Pack.
